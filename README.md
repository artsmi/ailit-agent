...
| **Граф архитектуры проекта + GUI «ailit memory» (workflow 7)** | [**`plan/7-workflow-project-architecture-graph.md`**](plan/7-workflow-project-architecture-graph.md) — **текущий рабочий workflow:** PAG (файл/папка, описание, рёбра, атрибуты узла), автоиндексация, `ailit memory` (Streamlit), `AgentMemory` / `AgentWork`, доноры (Graphiti, obsidian-memory-mcp, …). |
