...
| Область | Состояние |
|---------|-----------|
| **Workflow 13 (AgentMemory contract recovery)** | **Активен:** [`plan/13-agent-memory-contract-recovery.md`](plan/13-agent-memory-contract-recovery.md) — **G13.1–G13.4 закрыты** (G13.1–G13.3: как ранее; **G13.4:** `SemanticCNodeCandidate`, caps, `MemoryExtractorPromptBuilder`, окна remap ±100/200/500 + пороги policy, extractor JSON без full-B выше cap, `MemoryCNodePayloadV1` + strict `semantic_locator` в extractor result; тесты `tests/test_g13_semantic_c_identity.py`); далее G13.5+ (link claims, desktop store, integration). |
