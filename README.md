...
| **AgentMemory LLM + journal (workflow 11)** | [**`plan/11-agent-memory-llm-journal.md`**](plan/11-agent-memory-llm-journal.md) — **активно:** G11.3 добавляет AgentMemory LLM loop A→B→C со strict JSON policy, `max_memory_turns` и journal events без CoT. |
