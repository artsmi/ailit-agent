...
| **Workflow 14 (AgentMemory runtime W14, command protocol)** | **Открыт:** [`plan/14-agent-memory-runtime.md`](plan/14-agent-memory-runtime.md) — G14R.5 закрыт: `agent_memory_summary_service` (W14 LLM `summarize_c` / `summarize_b` → PAG, отпечатки); далее G14R.6+ по плану. |
