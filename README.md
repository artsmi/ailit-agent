...
| Область | Состояние |
|---------|-----------|
| **Workflow 13 (AgentMemory contract recovery)** | **Активен:** [`plan/13-agent-memory-contract-recovery.md`](plan/13-agent-memory-contract-recovery.md) — **G13.1–G13.2 закрыты** (G13.1: `PagGraphWriteService`, батч `pag.edge.upsert`, proto в `context/proto/runtime-event-contract.md`; G13.2: `MemoryLlmOptimizationPolicy`, `AgentMemoryQueryPipeline`, `memory.query_context` → planner → PAG grow → c_upserts через write service, тесты `tests/test_g13_agent_memory_llm_pipeline.py`); далее G13.3+ (feedback, C-schema, link claims, desktop store, integration). |
