...
| **AgentMemory LLM + journal (workflow 11)** | [**`plan/11-agent-memory-llm-journal.md`**](plan/11-agent-memory-llm-journal.md) — **активно:** G11.1 добавляет persistent `MemoryJournalStore` (`~/.ailit/runtime/memory-journal.jsonl`) и redacted structured journal rows. |
