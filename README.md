...
| Область | Состояние |
|---------|-----------|
| **Workflow 13 (AgentMemory contract recovery)** | **Активен:** [`plan/13-agent-memory-contract-recovery.md`](plan/13-agent-memory-contract-recovery.md) — **G13.1–G13.3 закрыты** (G13.1–G13.2: как ранее; **G13.3:** DTO `AgentWorkChangeFeedback`, `MemoryChangeUpdateService` + idempotency, `memory.change_feedback` в `AgentMemoryWorker`, AgentWork шлёт structured feedback вместо bare `memory.file_changed`, `SessionRunner` отдаёт `written_items` с `call_id`; тесты `tests/test_g13_agentwork_change_feedback.py`); далее G13.4+ (canonical C extraction schema, link claims, desktop store, integration). |
