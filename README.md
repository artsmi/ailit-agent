...
| Область | Состояние |
|---------|-----------|
| **Workflow 14 (AgentMemory planner command contract)** | **Открыт (G14.1–G14.6):** [`plan/14-agent-memory-planner-command-contract.md`](plan/14-agent-memory-planner-command-contract.md) — жёсткий JSON `agent_memory_planner.v1` с enum `op` (`repo_inventory`, `read_files`, …), dispatch в runtime, снятие двусмысленности `requested_reads`/namespace; DoD: тесты по именам из плана, обновление `context/proto`, smoke. |
| **Workflow 14 (AgentMemory runtime W14, command protocol)** | **Открыт:** [`plan/14-agent-memory-runtime.md`](plan/14-agent-memory-runtime.md) — G14R.5–G14R.7: summary service, quarantine legacy C extraction, сборка `agent_memory_result.v1` из `finish_decision` (c_summary/read_lines/b_path, гранты); далее G14R.8+ по плану. |
...
1. **AgentMemory / PAG / desktop (Workflow 14):** [`plan/14-agent-memory-planner-command-contract.md`](plan/14-agent-memory-planner-command-contract.md) — **открыт**; нормализация ответа планнера (команды `op` + `reason`) и детерминированный runtime. **Workflow 13** [`plan/13-agent-memory-contract-recovery.md`](plan/13-agent-memory-contract-recovery.md) — **закрыт**; канон контракта: `context/proto/runtime-event-contract.md`, карта UI: `context/arch/visual-monitoring-ui-map.md` (экран J). Workflow 12 — архивная ветка до recovery.
