...
| Токен-экономия и внешняя память (ветка) | **Порядок:** [`plan/workflow-token-economy-recipe.md`](plan/workflow-token-economy-recipe.md) — постановка **TE-0.0…TE-5.2** и handoff **§9** закрыты в документе (код **W-TE-1…4** — отдельно); далее [`plan/workflow-hybrid-memory-mcp.md`](plan/workflow-hybrid-memory-mcp.md) — H0…H4 закрыты в документе (в коде: local‑first KB scaffold + `kb_*` tools + `memory.access`); следующий файл — [`plan/workflow-memory-3.md`](plan/workflow-memory-3.md) (**M3-0…M3-5**). |
