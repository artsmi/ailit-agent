...
| Токен-экономия и внешняя память (ветка) | **Порядок:** сначала [`plan/workflow-token-economy-recipe.md`](plan/workflow-token-economy-recipe.md) (этапы **TE-0…TE-5**, постановка и история), затем по критерию перехода — [`plan/workflow-hybrid-memory-mcp.md`](plan/workflow-hybrid-memory-mcp.md) (этапы **H0…H4**, интеграция в `ailit`). |
...
3. **Токен-экономия и hybrid memory (если ведёте эту ветку):** строго **1 → 2**: [`plan/workflow-token-economy-recipe.md`](plan/workflow-token-economy-recipe.md), затем [`plan/workflow-hybrid-memory-mcp.md`](plan/workflow-hybrid-memory-mcp.md) (см. критерий перехода в первом файле).
