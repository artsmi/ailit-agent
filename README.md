...
| **AgentMemory LLM + journal (workflow 11)** | [**`plan/11-agent-memory-llm-journal.md`**](plan/11-agent-memory-llm-journal.md) — **активно:** G11.2 переводит `memory.query_context` на `AgentMemory:global`, расширяет response contract и пишет per-chat rows в общий journal. |
