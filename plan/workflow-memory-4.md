...
- ✅ **Задача M4-1.3 — Интеграция с permission/approval** (выполнено: ApprovalPending → skipped + telemetry)
 ...
 - ✅ **Задача M4-1.4 — Rate-limit авто‑KB (защита от “миллиона обращений”)** (выполнено: caps + `memory.auto_kb.rate_limited`)
  - **Описание:** ограничить частоту auto‑retrieval/auto‑fetch и auto‑write без участия модели.
    ...
    - `kb_fetch`: максимум **30** auto‑вызовов на сессию (для explainability/commit-exact)
    - `kb_write_fact`: максимум **10** auto‑вызовов на сессию (с учётом kinds)
  - **Критерии приёмки:**
    - при достижении лимита auto‑KB не ломает сессию, а эмитит событие
      `memory.auto_kb.rate_limited` (tool, cap, count, reason);
    - unified summary и UI показывают, что часть auto‑KB была rate-limited;
    - лимиты можно менять через env без правки кода.
  - **Проверки:**
    - unit‑тест на срабатывание лимита;
    - e2e не деградирует по времени (rate-limit не добавляет лишних LLM вызовов).
