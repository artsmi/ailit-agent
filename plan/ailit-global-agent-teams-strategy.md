...
4. **`max_turns` vs API** — пользователи путают лимит **шагов** оркестратора с `max_tokens` провайдера; без явного UX (этап N) растёт число «ложных багрепортов» при `max_turns_exceeded`.

---

## 12. Правила закрытия этапа (git, установка, команды для пользователя)

Эти правила **обязательны** для работы по данному документу с этого момента и для всех будущих этапов (G.1–M.1, N.1–N.3 и далее).
