...
Контекст (документы в полном дереве `plan/` в коммитах, где они есть: TE-рецепт, M4, perm‑5). В **этом** клоне в `plan/` могут лежать только согласованные ветки; см. также [`5-workflow-perm.md`](5-workflow-perm.md).
...
`read_file` с `offset`/`limit`, дедуп, `read_symbol` (Python) — `tools/agent_core/tool_runtime/builtins.py`, `python_read_symbol.py`, `read_file_text_slice` в `workdir_paths.py`.

Событие `fs.read_file.completed` с `range_read` и `unchanged_stub` пишет диагностику; агрегаты **`fs_read_file_range_calls`**, **`fs_read_file_duplicate_calls`** влияют на сигналы; оценка UI в `build_memory_efficiency_score` — см. `tools/ailit/token_economy_aggregates.py`.

---

## Доноры: ссылки на строки (как в M4-стиле)

### Claude Code / OpenCode (доноры)

Смотри пути в таблице **«Доноры»** выше (`claude-code`, `opencode` на диске разработчика) — без внешних URL, чтобы не держать непроверяемые ссылки.
...
**Критерии приёмки:** протокол согласован с рецептом token-economy / **анти-raw-dump**; явное правило «функция/класс (Python) → `read_symbol` при необходимости, иначе диапазон вокруг сигнатуры через `read_file` (R2)».
...
**Критерии приёмки:** на одном и том же workdir второй сценарий «точки входа» не обязан дублировать 6× `glob` до обращения к KB; подсказка KB-first в system-hints при `memory.enabled` снижает риск (e2e по JSONL — при следующей доработке).
...
**Критерии приёмки:** краткая строка в `docs/INDEX.md` (раздел про память); нет дублирования простыни M4.
...
**Критерии приёмки:** design note в `plan/`; реализация **не** входит в минимальное закрытие R4.1–R4.3. См. [`read-6-r4.4-memory-blocks-research.md`](read-6-r4.4-memory-blocks-research.md).
...
1. **Статус read-6 (этап закрытия):** R0, R1, R2, R3 (ручные сценарии), R4.1–R4.3 и R4.2 в доке, R4.4 design note — выполнено; строка в [`README.md`](../README.md) обновлена.
2. **Исчерпание workflow:** по [`.cursor/rules/project-workflow.mdc`](../.cursor/rules/project-workflow.mdc) — следующая итерация: **research** и **постановка** (например `plan/7-…` или отдельная ветка M5).
