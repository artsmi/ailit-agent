...
| A2 | `memory init` оркестрирует PREPARE→worker→VERIFY через **`AgentMemoryWorker` in-process** (`MemoryAgentConfig`, `broker_trace_stdout=False`), конверты `memory.query_context` с `memory_init: True`. **Broker не используется.** | `ailit/agent_memory/init/memory_init_orchestrator.py` |
| A3 | `memory query` аналогично in-process **`AgentMemoryWorker.handle`**, continuation-раунды в цикле. **Broker не используется.** | `ailit/agent_memory/query/memory_query_orchestrator.py` |
...
| D4 | **Разрешение broker endpoint:** (1) **`--broker-socket`** = путь к Unix socket; или (2) **`--memory-runtime-dir`** (или `AILIT_RUNTIME_DIR` / default) + опциональный **`--broker-chat-id`**. Если `chat_id` не задан и не задан **`--no-ensure-broker`**, CLI вычисляет стабильный короткий `chat_id` по `(project_root, PAG namespace)`, вызывает supervisor `brokers`, при отсутствии — **`create_or_get_broker`**. Явный **`--no-ensure-broker`** возвращает прежнюю политику: нужен `--broker-chat-id` и запись в registry (для CI/строгих сценариев). |
...
| D8 | Идентичность `chat_id` / `trace_id` / `broker_id` в envelope должны быть **согласованы** с broker config (`BrokerConfig.chat_id`, `broker_id` в ответах). Для CLI-сессии: переданный `--broker-chat-id`, либо стабильный короткий id вида `c` + 12 hex sha256 от `(project_root, namespace)` при auto-ensure. |
...
| G20.3 | D1, D7: `memory query` через broker; удаление/обход in-process пути по умолчанию; сохранение exit codes и stderr UX. **Anchors:** `ailit/agent_memory/query/memory_query_orchestrator.py`, `ailit/ailit_cli/memory_cli.py`, `ailit/ailit_cli/cli.py` argparse. **Сделано (2026-05-14):** обязательные `--broker-chat-id`, опционально `--broker-socket` / `--memory-runtime-dir`; `MemoryQueryOrchestrator.run(broker_invoke=…, broker_chat_id=…)`; клиент перенесён в `ailit_runtime/broker_json_client.py`, shim `ailit_cli/broker_json_client.py`. |
| G20.4 | D1, D3: новый `memory.index_project` (имя финализировать) в `memory_agent.py` + реализация в worker/общем helper; `cmd_memory_index` только вызывает broker RPC. **Anchors:** `ailit/ailit_cli/memory_cli.py`, `ailit/ailit_runtime/subprocess_agents/memory_agent.py`, `ailit/agent_memory/pag/pag_indexer.py`. **Сделано (2026-05-14):** `memory.index_project` в `AgentMemoryWorker._handle_index_project` → `index_project_to_default_store`; CLI `memory index` + флаги broker; тест `tests/runtime/test_memory_index_project_broker_g20.py`. |
| G20.5 | D1, D7: `memory init` через broker + continuation loop в CLI (логика раундов как сейчас, транспорт заменить). **Anchors:** `ailit/agent_memory/init/memory_init_orchestrator.py`, `ailit/ailit_cli/memory_cli.py`. **Сделано (2026-05-14):** обязательные `--broker-chat-id` / сокет/runtime; `MemoryInitOrchestrator.run(broker_invoke=…, broker_chat_id=…)`; payload `memory_init_shadow_journal_path` для журнала VERIFY; `AgentMemoryWorker` пишет init-журнал по этому пути при `memory_init`. |
| G20.6 | D5, D6: HTTP server + статика (две вкладки, brandbook `design/BRANDBOOK_CANDY.md`), `webbrowser.open`, `subscribe_trace` consumer; гибрид `pag-slice` при необходимости. **Anchors:** `ailit/ailit_cli/memory_cli.py`, новый каталог static (например `ailit/ailit_cli/static/memory_viz/`). **Сделано (2026-05-14):** `memory_viz_runtime.py` (`MemoryVizRuntime`, whitelist trace, `/api/pag-slice`), `static/memory_viz/index.html` (Trace | PAG), флаги `--no-memory-viz` / `--without-memory-viz` на index/init/query. |
| G20.7 | Регрессия: pytest + flake8; **проверка** без `AgentMemoryWorker(` в `ailit/agent_memory/init/memory_init_orchestrator.py` / `ailit/agent_memory/query/memory_query_orchestrator.py` (whitelist в docstring `tests/runtime/test_g20_7_orchestrator_contract.py`). **Сделано (2026-05-14):** тот тест + `pytest`/`flake8` по G20-путям; ручной поиск: ``grep -E 'AgentMemoryWorker\\s*\\('`` по двум файлам — пусто (или `rg` при установленном ripgrep). |
