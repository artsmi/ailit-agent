...
| G20.4 | D1, D3: новый `memory.index_project` (имя финализировать) в `memory_agent.py` + реализация в worker/общем helper; `cmd_memory_index` только вызывает broker RPC. **Anchors:** `memory_cli.py`, `memory_agent.py`, `pag_indexer.py`. **Сделано (2026-05-14):** `memory.index_project` в `AgentMemoryWorker._handle_index_project` → `index_project_to_default_store`; CLI `memory index` + флаги broker; тест `tests/runtime/test_memory_index_project_broker_g20.py`. |
| G20.5 | D1, D7: `memory init` через broker + continuation loop в CLI (логика раундов как сейчас, транспорт заменить). **Anchors:** `memory_init_orchestrator.py`, `memory_cli.py`. **Сделано (2026-05-14):** обязательные `--broker-chat-id` / сокет/runtime; `MemoryInitOrchestrator.run(broker_invoke=…, broker_chat_id=…)`; payload `memory_init_shadow_journal_path` для журнала VERIFY; `AgentMemoryWorker` пишет init-журнал по этому пути при `memory_init`. |
| G20.6 | D5, D6: HTTP server + статика (две вкладки, brandbook `design/BRANDBOOK_CANDY.md`), `webbrowser.open`, `subscribe_trace` consumer; гибрид `pag-slice` при необходимости. **Anchors:** `memory_cli.py`, новый каталог static (например `ailit/ailit_cli/static/memory_viz/`). **Сделано (2026-05-14):** `memory_viz_runtime.py` (`MemoryVizRuntime`, whitelist trace, `/api/pag-slice`), `static/memory_viz/index.html` (Trace | PAG), флаги `--no-memory-viz` / `--without-memory-viz` на index/init/query. |
| G20.7 | Регрессия: pytest + flake8; **проверка** без `AgentMemoryWorker(` в `memory_init_orchestrator` / `memory_query_orchestrator` (whitelist в docstring `tests/runtime/test_g20_7_orchestrator_contract.py`). **Сделано (2026-05-14):** тот тест + `pytest`/`flake8` по G20-путям; ручной поиск: ``grep -E 'AgentMemoryWorker\\s*\\('`` по двум файлам — пусто (или `rg` при установленном ripgrep). |
