...
def _bootstrap_repo_venv_if_needed(argv: list[str]) -> None:
    ...
    os.execve(str(py), [str(py), "-m", "ailit_cli.cli", *argv], env)
...
def _cmd_agent_usage_last(args: argparse.Namespace) -> int:
    ...
    from ailit_cli.agent_usage_cli import print_last_usage_from_log
...
def _cmd_agent_token_econ_report(args: argparse.Namespace) -> int:
    ...
    from ailit_cli.agent_token_econ_cli import (
...
def _cmd_session_usage_list(_args: argparse.Namespace) -> int:
    ...
    from ailit_cli.session_usage_cli import print_session_list
...
def _cmd_session_usage_show(args: argparse.Namespace) -> int:
    ...
    from ailit_cli.session_usage_cli import print_session_show
...
def _cmd_session_usage_summary(args: argparse.Namespace) -> int:
    ...
    from ailit_cli.session_usage_cli import print_session_summary
...
def _make_cmd_session_usage_subsystem(subsystem: str):
    ...
    def _cmd(args: argparse.Namespace) -> int:
        ...
        from ailit_cli.session_usage_cli import print_session_subsystem
...
def _cmd_agent_run(args: argparse.Namespace) -> int:
    ...
    from ailit_cli.agent_provider_config import AgentRunProviderConfigBuilder
    from ailit_cli.task_spec import RunTaskArtifactWriter, TaskSpecResolver
    from ailit_cli.process_log import ensure_process_log
    ...
    from ailit_cli.defaults_resolver import DefaultProviderModelResolver
...
def _cmd_compat_run(args: argparse.Namespace) -> int:
    ...
    from ailit_cli.compat_adapter import run_compat_workflow
    from ailit_cli.process_log import ensure_process_log
...
def _cmd_debug_bundle(args: argparse.Namespace) -> int:
    ...
    from ailit_cli.debug_bundle import build_debug_bundle
...
def _cmd_doctor_paths(_args: argparse.Namespace) -> int:
    ...
    from ailit_cli.doctor_cli import cmd_doctor_paths
...
def _cmd_doctor_data_policy(_args: argparse.Namespace) -> int:
    ...
    from ailit_cli.doctor_cli import cmd_doctor_data_policy
...
def main(argv: list[str] | None = None) -> int:
    ...
    from ailit_cli.config_cli import register_config_parser
    from ailit_cli.kb_cli import (
        cmd_kb_dump_audit,
        cmd_kb_rebuild_index,
        cmd_kb_ttl_apply,
    )
    from ailit_cli.memory_cli import (
        cmd_memory_index,
        cmd_memory_init,
        cmd_memory_pag_slice,
        cmd_memory_query,
    )
    from ailit_cli.models_cli import register_models_parser
    from ailit_cli.desktop_cli import register_desktop_parser
    from ailit_cli.project_cli import register_project_parser
    from ailit_cli.runtime_cli import register_runtime_parser
    from ailit_cli.setup_cli import register_setup_parser
    ...
    p_agent = sub.add_parser(
        "agent",
        help="Запуск workflow и утилиты agent (без интерактивного TUI)",
    )
