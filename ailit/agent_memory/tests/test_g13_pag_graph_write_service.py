...
from agent_memory.pag.pag_graph_write_service import (
...
def _files_with_regex(
    ...
    for path in sorted(ac.rglob("*.py")):
        if not path.is_file():
            continue
        if "/agent_memory/tests/" in path.as_posix():
            continue
        try:
...
def test_runtime_direct_upsert_is_guarded() -> None:
    ...
    assert rel == {
        "ailit/agent_memory/pag/pag_graph_write_service.py",
    }
    assert "agent_memory.pag.pag_indexer" in OFFLINE_PAG_WRITER_MODULES
...
def test_rg_upsert_call_sites_match_plan_whitelist() -> None:
    ...
    expected = {
        "ailit/agent_memory/storage/sqlite_pag.py",
        "ailit/agent_memory/pag/pag_graph_write_service.py",
        "ailit/agent_memory/pag/pag_indexer.py",
        "ailit/agent_memory/pag/d_creation_policy.py",
        "ailit/agent_memory/pag/link_claim_resolver.py",
        "ailit/agent_memory/services/memory_c_remap.py",
        "ailit/agent_memory/services/memory_growth.py",
        "ailit/agent_memory/services/agent_memory_summary_service.py",
        "ailit/agent_memory/query/agent_memory_query_pipeline.py",
        "ailit/agent_memory/services/agent_memory_link_candidate_validator.py",
