...
from agent_memory.pag.pag_indexer import ChangedRange, PagIndexer
...
class PagRuntimeAgentMemory:
    ...
    def build_slice_for_goal(
        ...
        *,
        ...
        from agent_work.session.repo_context import (  # noqa: WPS433
            detect_repo_context,
            namespace_for_repo,
        )
        ...
        *,
        ...
        def score(n: PagNode) -> int:
            ...
            if p.endswith(("cli.py", "main.py", "app.py")):
                s += 10
            if "/ailit/ailit_cli/" in p or p.startswith("ailit/ailit_cli/"):
                s += 4
            if "/ailit/agent_memory/" in p or p.startswith(
                "ailit/agent_memory/",
            ):
                s += 3
            if p.endswith(".py"):
