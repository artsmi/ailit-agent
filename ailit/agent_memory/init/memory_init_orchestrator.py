...
from agent_memory.cli.memory_init_cli_outcome import (
...
from agent_memory.init.memory_init_transaction import (
    MemoryInitPaths,
    ...
    def run(
        ...
        *,
        ...
        try:
            ...
            max_continuation_rounds_effective = max_continuation_rounds
            ...
            while True:
                ...
                try:
                    ...
                    self._emit_memory_init_user_summary(
                        ...
                        max_continuation_rounds_effective=(
                            max_continuation_rounds_effective
                        ),
                    )
                    return memory_init_exit_code(
                ...
                # Init soak: без ``memory_continuation_required`` envelope
                # может быть ``partial`` или ``complete``, а shadow-журнал ещё
                # без финального complete (например summarize_c.apply_failed).
                # Пока VERIFY после RPC ложен, продолжаем при ``!= blocked``.
                if not cont:
