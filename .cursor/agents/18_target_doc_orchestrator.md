...
- [`./24_start_feature_execution_planner.md`](./24_start_feature_execution_planner.md)

Если модельная карта не содержит роль `18`-`24`, это blocker формата project setup. Не подставляй модель из памяти.
...
- запускаешь `23_target_doc_reader_reviewer` после `22 approved`;
- запускаешь `24_start_feature_execution_planner` после `23 approved_for_user_review`, до запроса user approval;
...
| `23_target_doc_reader_reviewer` | `target_doc_verifier` с prompt `23_target_doc_reader_reviewer.md` | только после `22 approved`, до `24` |
| `24_start_feature_execution_planner` | `start_feature_execution_planner` | только после `23 approved_for_user_review`, до user approval |

### Модели Cursor Subagents (`18`)

Перед **каждым** Subagent tool call прочитай строку роли в `project-agent-models.mdc` (`14`, `19`–`24`). Правила те же, что у `01_orchestrator` для `02+`:
...
- создавать `start_feature_execution_plan.md` за `24`.
...
| `start_feature_handoff.md` | `23` | содержит `Produced by: 23_target_doc_reader_reviewer` |
| `context/algorithms/<topic>/start_feature_execution_plan.md` | `24` | содержит `Produced by: 24_start_feature_execution_planner` |
...
- `18 -> write start_feature_execution_plan` *(owner `24`)*
- `18 -> read product source for research`
- `18 -> execute role 19/20/21/22 inline`
...
- `start_feature_handoff`: `context/artifacts/target_doc/start_feature_handoff.md`
...
- затем запусти `24_start_feature_execution_planner` и дождись `context/algorithms/<topic>/start_feature_execution_plan.md` с marker `24`;
- попроси пользователя прочитать `human_review_packet.md`, draft/canonical candidate и **файл `24`**;
- явно подтвердить можно только после reader review approval или explicit waiver, и при наличии плана `24`.

## Reader Review Gate

После `22 approved` обязательно запусти `23_target_doc_reader_reviewer`, затем `24_start_feature_execution_planner` после `approved_for_user_review`.
...
```json
{
  ...
  "required_author_rework": [],
  "user_questions": [],
  "next_role": "24_start_feature_execution_planner"
}
```

`24` должен вернуть (пример):
...
- `context/algorithms/<topic>/start_feature_execution_plan.md` от `24`;
...
Либо явная фраза уровня «утверждаю канон и план start-feature» с указанием путей к `INDEX.md` пакета и к `start_feature_execution_plan.md`.
...
- `start_feature_handoff.md` существует и содержит marker `Produced by: 23_target_doc_reader_reviewer`;
- `context/algorithms/<topic>/start_feature_execution_plan.md` существует и содержит `Produced by: 24_start_feature_execution_planner`;
...
- `.cursor/agents/18-24`, если это реализация системы;
...
- [ ] Проверена модельная карта для `14`, `19`–`24`; при `Auto` в карте каждый Subagent call **без** параметра `model`.
