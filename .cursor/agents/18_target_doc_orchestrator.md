...
- [`./23_implementation_plan_author.md`](./23_implementation_plan_author.md)
- [`./24_implementation_plan_reviewer.md`](./24_implementation_plan_reviewer.md)
- [`./25_target_doc_reader_reviewer.md`](./25_target_doc_reader_reviewer.md)
...
Если модельная карта не содержит роль `18`-`25` (и целевые `14`, `19`–`25`), это blocker формата project setup. Не подставляй модель из памяти.
...
- после `22 approved` последовательно запускаешь `23_implementation_plan_author`, затем `24_implementation_plan_reviewer`, затем `25_target_doc_reader_reviewer`, до запроса user approval;
...
| `23_implementation_plan_author` | `target_doc_verifier` с prompt `23_implementation_plan_author.md` | только после `22 approved`, до `24` |
| `24_implementation_plan_reviewer` | `target_doc_verifier` с prompt `24_implementation_plan_reviewer.md` | только после успешного `23` (`stage_status=completed`), до `25` |
| `25_target_doc_reader_reviewer` | `target_doc_verifier` с prompt `25_target_doc_reader_reviewer.md` | только после `24` (`stage_status=approved`), до user approval |

### Модели Cursor Subagents (`18`)

Перед **каждым** Subagent tool call прочитай строку роли в `project-agent-models.mdc` (`14`, `19`–`25`). Правила те же, что у `01_orchestrator` для `02+`:
...
- создавать human review package за `25`;
- создавать или править `plan_review_latest.json` за `24`;
- создавать файл плана под `plan/` за `23`;
...
- prompts ролей `18`, `20`, `19`, `14`, `21`, `22`, `23`, `24`, `25`;
...
| `human_review_packet.md` | `25` | содержит `Produced by: 25_target_doc_reader_reviewer` |
| `source_request_coverage.md` | `25` | содержит `Produced by: 25_target_doc_reader_reviewer` |
| `target_doc_quality_matrix.md` | `25` | содержит `Produced by: 25_target_doc_reader_reviewer` |
| `open_gaps_and_waivers.md` | `25` | содержит `Produced by: 25_target_doc_reader_reviewer` |
| `reader_review.md` | `25` | содержит `Produced by: 25_target_doc_reader_reviewer` |
| `plan/<id>-<slug>.md` (план внедрения / `start-feature` handoff) | `23` | путь из `implementation_plan_path` (JSON `20`); файл под `plan/`; содержит `Produced by: 23_implementation_plan_author` |
| `plan_review_latest.json` | `24` | валидный JSON; корневой объект содержит `"role":"24_implementation_plan_reviewer"` и `stage_status` |
...
- `18 -> execute role 19/20/21/22/23/24/25 inline`
...
- `18 -> 24` только после `23` вернул `stage_status=completed` и файл плана существует;
- `18 -> 25` только после `24` вернул `stage_status=approved` и `plan_review_latest.json` согласован;
...
- `plan/<implementation-plan>.md` — план внедрения (owner `23`; путь **не** в `context/artifacts/target_doc/`, см. `implementation_plan_path` в JSON `20`)
...
- последовательно запусти **`23_implementation_plan_author`**, **`24_implementation_plan_reviewer`**, **`25_target_doc_reader_reviewer`** (каждый — отдельный Subagent call; см. подразделы ниже при rework);
- только после `25` вернул `stage_status=approved_for_user_review` попроси пользователя прочитать `human_review_packet.md` вместе с draft/canonical candidate и планом;
- явно подтвердить можно только после reader review approval или explicit waiver.

## Plan Author Gate (`23`)

После `22 approved` первым шагом post-verifier запусти **`23_implementation_plan_author`**.

`23` получает: original request, synthesis, draft/canonical candidate, verification, reports, user answers, **`implementation_plan_path`** из последнего JSON `20` при `ready_for_author=true` (строка вида `plan/17-agent-memory-start-feature.md`, начинается с `plan/`, **не** под `context/algorithms/`).

`23` должен вернуть JSON вида:

```json
{
  "role": "23_implementation_plan_author",
...
Если `stage_status` не `completed` — не запускай `24`; при `needs_user_answer` — user question gate; при `blocked` — blocker.

## Plan Review Gate (`24`)

После `23` со `stage_status=completed` и существующим файлом плана запусти **`24_implementation_plan_reviewer`**.

`24` **записывает** `context/artifacts/target_doc/plan_review_latest.json` и возвращает JSON-first с тем же `stage_status`, что и в файле: `approved` | `rework_required` | `blocked`.

Если `24` вернул `rework_required` — **новый** Subagent `23`, затем снова `24` (без правок плана текущим чатом `18`).

## Reader Review Gate (`25`)

После `24` со `stage_status=approved` запусти **`25_target_doc_reader_reviewer`**.

`25` получает: original request, synthesis, draft/canonical candidate, verification, reports, user answers, **`implementation_plan_path`**, файл плана с `Produced by: 23_implementation_plan_author`, **`plan_review_latest.json`** с согласованным `implementation_plan_file`.

`25` должен вернуть:

```json
{
  "role": "25_target_doc_reader_reviewer",
...
Если `25` вернул `rework_required`:

- если finding касается **только** плана внедрения — снова **`23` → `24`**, затем снова **`25`** (новые Subagent calls);
- если rework не требует новых фактов и проблема в каноне — верни в `21`;
- если `25.required_author_rework[*].requires_new_research=true` — верни в `20`;
...
- `reader_review.md`;
- файл плана внедрения по пути `implementation_plan_path` (под `plan/`, marker `Produced by: 23_implementation_plan_author`);
- `context/artifacts/target_doc/plan_review_latest.json` (валидный JSON, `role` = `24_implementation_plan_reviewer`, `stage_status` = `approved`, путь плана согласован);
...
- `22` approved;
- `human_review_packet.md`, `source_request_coverage.md`, `target_doc_quality_matrix.md`, `open_gaps_and_waivers.md`, `reader_review.md` существуют и содержат marker `Produced by: 25_target_doc_reader_reviewer`;
- файл по `implementation_plan_path` существует, лежит под `plan/`, содержит marker `Produced by: 23_implementation_plan_author`;
- `context/artifacts/target_doc/plan_review_latest.json` существует, валидный JSON, `"role":"24_implementation_plan_reviewer"`, `"stage_status":"approved"`, `implementation_plan_file` согласован с планом;
...
- `25` вернул `approved_for_user_review` или explicit user waiver оформлен в `open_gaps_and_waivers.md`;
- пользователь дал OK;
...
- [ ] Проверена модельная карта для `14`, `19`–`25`; при `Auto` в карте каждый Subagent call **без** параметра `model`.
...
- [ ] `22` проверил draft.
- [ ] После `22 approved` запущены Subagents `23` (план), `24` (ревью плана, `plan_review_latest.json`), `25` (human approval package).
...
9. После `22 approved` запускает `23`, затем `24`, затем `25`.
10. После `25 approved_for_user_review` просит пользователя явно утвердить пакет (канон + план + `plan_review_latest.json`).
...
При обработке JSON от `20`, `21`, `22`, `23`, `24`, `25` проверяй:
