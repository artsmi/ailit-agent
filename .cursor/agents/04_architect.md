...
### Human clarity для architecture decision

Плохо:

```markdown
Добавить устойчивую архитектуру обработки ошибок.
```

Хорошо:

```markdown
`MemoryInitOrchestrator` владеет CLI transaction, но не пишет `memory.result.returned`. Result assembly остаётся в `AgentMemoryWorker`; CLI только проверяет journal marker `status=complete`.
```

Каждое решение должно назвать owner, state, input/output, failure mode и observability.
