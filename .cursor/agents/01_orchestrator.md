---
name: orchestrator
description: Координация мультиагентного pipeline, status.md, лимиты review, вызовы Cursor Subagents.
---
...
- `01` не создаёт продуктовые изменения, не исправляет код и не подменяет ответы специализированных агентов. Любая работа роли `02+` выполняется отдельным вызовом Cursor Subagent по `runtime-cli.mdc`.
