...
- **`implementation_plan_path`** из JSON `20` / handoff `18` (обязательно): строка `plan/<NN>-<slug>.md`, начинается с `plan/`, **не** под `context/algorithms/`.
...
- **файл плана внедрения** по пути **`implementation_plan_path`** (каталог `plan/`, не `context/algorithms/**`, не `context/artifacts/target_doc/`)

Каждый файл **кроме** `plan/…` (см. ниже про marker) должен содержать marker:

```markdown
Produced by: 23_target_doc_reader_reviewer
```

Файл под `plan/` обязан содержать строку `Produced by: 23_target_doc_reader_reviewer` в теле (обычно после заголовка идентификатора процесса).
...
## План внедрения (`plan/`, бывший start-feature handoff)

Файл по пути **`implementation_plan_path`** — мост от утверждённого канона к будущему `start-feature`. Канон **не** содержит планов; каждая строка плана ссылается на разделы канона.

Обязательная структура (ориентир по жёстким границам: [`plan/14-agent-memory-runtime.md`](../../../plan/14-agent-memory-runtime.md)):

```markdown
# Рабочий процесс NN: …

**Идентификатор:** … (файл `plan/NN-….md`).

**Канон (SoT):** ссылка на `context/algorithms/<topic>/INDEX.md` и файлы пакета.

Produced by: 23_target_doc_reader_reviewer

## 1. Цель и границы
## 2. Обязательные требования к плану (трассировка к канону)
## 3. Трассировка: слайсы / этапы → канон (таблица со ссылками на markdown канона)
## 4. Запрещённый слишком широкий scope
## 5. Входы для start-feature
## 6. Ожидаемые доказательства (с ссылками на разделы канона)
## 7. Известные gaps
```

Для большого target doc таблица **«Трассировка: слайсы → канон»** обязательна: у каждой строки — хотя бы одна ссылка на файл канона. Если `20` не дал small-scope recommendations, `23` должен либо вывести rework к `20`, либо сформулировать reader-facing blocker.
...
- каждый файл под `context/artifacts/target_doc/` из списка выше содержит marker `Produced by: 23_target_doc_reader_reviewer`;
- файл плана под `plan/` содержит строку `Produced by: 23_target_doc_reader_reviewer`;
...
- файл по `implementation_plan_path` существует, лежит под `plan/`, содержит таблицу трассировки к канону и marker `Produced by: 23_target_doc_reader_reviewer`;
...
- для крупного канона в `implementation_plan` есть непустая таблица «слайсы → канон» (или эквивалент в §3 шаблона);
...
- `18` включает в сообщение approval путь к канону и путь к **`implementation_plan_file`** (`plan/…`).
- `18` sends ntfy.
...
- [ ] Файл по `implementation_plan_path` создан под `plan/`.
- [ ] File/link/provenance checks passed.
...
7. Составь файл плана внедрения по **`implementation_plan_path`** (содержимое по шаблону §«План внедрения» выше).
