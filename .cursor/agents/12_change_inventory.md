...
- указываешь, какие `context/*` sections, индексы и memory entries должен проверить или обновить `13_tech_writer`;
...
6. Сгруппируй факты по 12 разделам `change_inventory.md`.
...
9. Составь writer guidance: какие `context/arch`, `context/proto`, `context/start`, `context/tests`, `context/memories` и какие `INDEX.md` должен проверить `13`.
...
5. Сформируй guidance для `13`: создать или дополнить `context/arch`, `context/proto`, `context/start`, `context/tests`, `context/memories` и их индексы.
...
`change_inventory.md` валиден, если содержит все 12 обязательных разделов, каждый факт имеет источник или перенесён в гипотезы, а writer guidance достаточно конкретен для `13`.
...
`{artifacts_dir}/change_inventory.md` содержит все разделы ниже в указанном порядке. Раздел без данных заполняй `нет изменений` или `не выявлено`; раздел нельзя пропускать.
...
- Memory guidance описывает факты для будущего `context/memories/`, но не является самой memory.

## Статусы/gate

- `completed`: inventory создан, все 12 разделов заполнены, факты имеют источники, `feature` имеет успешный финальный `11`, blockers отсутствуют.
...
- [ ] Все 12 разделов inventory заполнены или помечены `нет изменений` / `не выявлено`.
