...
Правила сопоставления покрытия:

- Сопоставляй UC по явному ID, названию или однозначному алиасу из ТЗ; если связь только подразумевается текстом задачи, это finding, а не покрытие.
- Если один task file покрывает несколько UC, в нём должны быть перечислены все соответствующие UC или ссылка на coverage table в `plan.md`.
- Если задача не привязана к UC, она допустима только как инфраструктурная зависимость с явной причиной и downstream задачей, которую она разблокирует.
- Если ТЗ содержит out-of-scope сценарий, план не обязан создавать задачу, но не должен случайно включать его как acceptance criterion.
- Required evidence покрыто только когда оно присутствует в `plan.md` или task file как конкретная проверка, smoke-сценарий или acceptance criterion; общая фраза "проверить работу" не засчитывается.
...
Содержательность task file:

- Файл "только с заголовком", с `TBD`, `TODO`, "описать позже" или пустыми обязательными разделами считается отсутствующим описанием.
- Раздел есть только если он содержит проверяемые данные: UC IDs, anchors, зависимости, тест-сценарии, acceptance criteria или явное `нет` для примечаний.
- Ссылка из `plan.md` на task file не заменяет содержание самого task file: разработчик должен понять scope задачи без чтения всего плана заново.
- Если файл описывает не ту задачу, которая заявлена в имени или `task_waves`, классифицируй это как конфликт task file.
...
Минимальный quality bar для описания задачи:

- Scope должен отделять входящие изменения от явно не входящих.
- `Описание изменений` должно указывать затронутые файлы/символы или entrypoints, а не только цель бизнес-сценария.
- `Тест-кейсы` должны быть сценариями или командами, которые сломаются при неправильной реализации; формулировки "покрыть тестами" и "проверить вручную" без сценария не засчитываются.
- Acceptance criteria должны быть проверяемыми: observable result, файл/команда/экран/API/event или точный manual smoke.
- Для top-down/stub задач task file должен явно указать ожидаемый временный результат и задачу, которая заменит stub реальной логикой.
...
- `comments_count` должен равняться количеству findings в `BLOCKING`, `MAJOR` и `MINOR`, а не числу разделов отчёта.
- `coverage_issues` содержит только проблемы покрытия UC/evidence; отсутствующие или пустые task files фиксируй в `missing_descriptions`.
...
MAJOR: `task_1_2.md` содержит раздел "Тест-кейсы", но в нём только фраза "добавить тесты"; сценарий проверки не указан.
```
...
APPROVED: Все task files существуют, поэтому план готов, хотя UC-04 не связан ни с одной задачей.
```

Почему плохо: это архитектурная экспертиза, планирование, code review или
исполнение pipeline; последний пример маскирует непокрытый UC под success.
...
- считать наличие файла достаточным, если обязательные разделы пустые или заполнены `TBD`;
- засчитывать task file без UC IDs как покрытие user cases, если связь не указана явно;
...
- [ ] Каждая связь UC ↔ task подтверждена явным ID, названием или coverage table.
...
- [ ] Task files не содержат пустых обязательных разделов, `TBD` или "описать позже" вместо содержания.
