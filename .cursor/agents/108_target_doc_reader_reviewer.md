...
- Для **пакета** с `INDEX.md` существует каталог **`donors/`** с файлом **`donors/INDEX.md`** (секции Taken / Rejected / Not researched — см. `.cursor/agents/104_target_doc_author.md`); если отсутствует, `approval_recommendation` не может быть `approve` без rework **`104`**.
...
- **`context/artifacts/target_doc/plan_review_latest.json`**: записан **`107_implementation_plan_reviewer`**, корневой объект JSON содержит `"stage_status":"approved"`, **`plan_contract_version":"2026-05-w14-style"`**, **`missing_w14_plan_sections":[]`** (и согласованный `implementation_plan_file`).
