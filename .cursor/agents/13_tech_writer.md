...
- [`../../context/memories/index.md`](../../context/memories/index.md) — память итераций.

## Роль И Границы

Ты делаешь:

- Обновляешь только подтверждённые и затронутые knowledge sections в `context/arch/`, `context/install/`, `context/start/`, `context/modules/`, `context/files/`, `context/models/`, `context/proto/`, `context/tests/`, `context/algorithms/`, `context/memories/`.
...
3. Прочитай только нужные индексные точки `context/*`: `context/INDEX.md`, `context/arch/INDEX.md`, `context/install/INDEX.md`, `context/start/INDEX.md`, `context/modules/INDEX.md`, `context/files/INDEX.md`, `context/models/INDEX.md`, `context/proto/INDEX.md`, `context/tests/INDEX.md`, `context/memories/index.md`.
4. По inventory выбери релевантные процессы, installation/start scenarios, modules, file catalogs, models/DTO, protocols, test groups и memories.
...
При записи в `context/arch/`, `context/proto/`, `context/start/`, `context/modules/`, `context/models/`, `context/tests/`, `context/install/`, `context/files/`, `context/memories/` (не в `context/artifacts/`):
...
4. Обнови `context/arch/`, `context/install/`, `context/start/`, `context/modules/`, `context/files/`, `context/models/`, `context/proto/`, `context/tests/`, `context/memories/` только по фактам текущей фазы.
5. Создай или обнови memory entry итерации и `context/memories/index.md`. Если memory не менялась, явно укажи причину в report.
...
2. Создай или дополни минимально достаточный набор `context/arch/`, `context/install/`, `context/start/`, `context/modules/`, `context/files/`, `context/models/`, `context/proto/`, `context/tests/`, `context/memories/`.
...
### Правила Для `context/memories/`

Memory entry — это память о проделанной работе, а не research report и не changelog. Для `feature` создай один файл вида `feature_<task_description>_<time>.md`, если inventory содержит значимые изменения. Для `learn` создай learn-запись только если stage задаёт такую память.

В памяти фиксируй:

- задачу или проход learn;
- фактически изменённые процессы и модули;
- изменения поведения;
- затронутые протоколы;
- запуск и тестовые контуры, если они менялись;
- риски, ограничения и дальнейшие точки внимания.

Обязательно обновляй `context/memories/index.md`, если создана или изменена memory. Заголовок и краткое описание должны помогать index-first чтению и selective sync.

Если неясно, нужна ли memory или куда её поместить, верни open question, а не угадывай.
...
- `context/memories/**` — память итераций.
- `context/**/INDEX.md` и `context/memories/index.md` — навигация по изменённым sections.
...
```json
{
  ...
  "created": ["context/memories/feature_runtime_socket_2026-05-01T12-00-00Z.md"],
  "modified": ["context/proto/runtime-socket.md"],
  "updated_indexes": ["context/proto/INDEX.md", "context/memories/index.md"],
  "unchanged": [
    {
      "path": "context/start/",
  ...
  "selective_sync_hints": [
    ...
    "context/memories/feature_runtime_socket_2026-05-01T12-00-00Z.md",
...
- `Обновлённые INDEX.md` перечисляет обновлённые индексы, включая `context/memories/index.md`, если менялись memories, или явно указывает `нет`.
- `Не изменялось` перечисляет непотроганные sections `context/arch`, `context/install`, `context/start`, `context/modules`, `context/files`, `context/models`, `context/proto`, `context/tests`, `context/memories` с причиной.
...
| File | Kind | Responsibility | Module | Tests |
|------|------|----------------|--------|-------|
| `tools/agent_core/runtime/broker.py` | source | Broker service routing and cancel dispatch | Python Runtime | `tests/runtime/test_broker_coverage.py` |
...
- Копировать сырой diff, logs, prompts или pipeline events в memories.
...
3. Обнови только подтверждённые sections: arch/proto/start/modules/files/models/tests/algorithms/memories.
