...
- Обновляешь только подтверждённые и затронутые knowledge sections в `context/arch/`, `context/proto/`, `context/start/`, `context/tests/`, `context/memories/`.
...
1. Режим и краткий контекст: `feature` или `learn`, источник задачи, границы анализа.
...
8. Главные файлы с кратким пояснением роли каждого.
9. Пробелы, гипотезы и допущения, явно отделённые от фактов.
10. Knowledge sections для обновления: `context/arch`, `context/proto`, `context/start`, `context/tests`, `context/memories`.
11. Индексные файлы для проверки или обновления: `INDEX.md`, `context/memories/index.md`.
12. Selective sync hints для производного DB index, если проект его поддерживает.
...
3. Прочитай только нужные индексные точки `context/*`: `context/INDEX.md`, `context/arch/INDEX.md`, `context/proto/INDEX.md`, `context/start/INDEX.md`, `context/tests/INDEX.md`, `context/memories/index.md`.
4. По inventory выбери релевантные процессы, протоколы, запуск, тестовые группы и memories.
...
4. Обнови `context/arch/`, `context/proto/`, `context/start/`, `context/tests/`, `context/memories/` только по фактам текущей фазы.
...
2. Создай или дополни минимально достаточный набор `context/arch/`, `context/proto/`, `context/start/`, `context/tests/`, `context/memories/`.
...
- `Не изменялось` перечисляет непотроганные sections `context/arch`, `context/proto`, `context/start`, `context/tests`, `context/memories` с причиной.
...
```json
{
  ...
  "change_inventory": "docs/implementation/change_inventory.md",
  "tech_writer_report": "docs/implementation/tech_writer_report.md",
  "created": [],
...
- [ ] Inventory содержит 12 обязательных разделов или возвращён blocker.
