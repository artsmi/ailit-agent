...
| `plan_review_latest.json` | `107` | валидный JSON; корневой объект содержит `"role":"107_implementation_plan_reviewer"`, `stage_status`, **`plan_contract_version":"2026-05-w14-style"`** при `approved`, **`missing_w14_plan_sections`** (массив; при `approved` обязан быть `[]`) |
...
- `100 -> 108` только после `107` вернул `stage_status=approved`, файл `plan_review_latest.json` на диске содержит **`plan_contract_version":"2026-05-w14-style"`** и **`missing_w14_plan_sections":[]`**, и JSON ответа Subagent `107` согласован с файлом;
...
## Implementation plan contract (Plan quality bar for `106`)

Перед **каждым** вызовом Subagent **`106_implementation_plan_author`** включи в handoff (в конец сообщения Subagent) блок **verbatim** ниже — это **Plan quality bar**; `106` обязан отразить его в структуре плана (см. `.cursor/agents/106_implementation_plan_author.md`). **`100` не пишет** файл плана и **не** читает `plan/**` для проверки содержимого: структурная приёмка — у **`107`**; `100` проверяет только JSON **`plan_review_latest.json`** и ответ Subagent **`107`**.

```markdown
## Plan quality bar (W14-style) — обязательно для файла плана

Опора: раздел «Качество планов» в `project-workflow.mdc` (п. 1–13 и self-review bullets).

Обязательные разделы и элементы:
1. «Реализация и архитектурный сдвиг» (кратко продукт + граница архитектуры).
2. Цель, границы, аудит, нормативные id → решения.
3. Таблица «Трассировка нормативных ID → этап» (все id из контрактов привязаны к Gx; orphan id запрещён).
4. «Запуск start-feature» или §0 при наличии anchors в product-коде; иначе явное `n/a (doc-only)` с причиной.
5. Каждый этап Gx: строка `Обязательные описания/выводы:`, anchors, зависимости, anti-patterns, приёмка с **именами** тестов/команд.
6. Сводный § тестов и статики; observability и config SoT или **`n/a`** с причиной.
7. «Инструкции команде разработки: donor» + таблица «Donor ref (минимальная таблица)»; без копипаста кода из donor.
8. DoD / трассировка слайс → канон; self-review плана (таблица ok / n/a по критериям `project-workflow.mdc`).
```
...
`106` получает: original request, synthesis, draft/canonical candidate, verification, reports, user answers, **`implementation_plan_path`** из последнего JSON `103` при `ready_for_author=true` (строка вида `plan/17-agent-memory-start-feature.md`, начинается с `plan/`, **не** под `context/algorithms/`), плюс **в конце handoff** блок **Plan quality bar** из раздела **«Implementation plan contract (Plan quality bar for `106`)»** этого файла (verbatim).
...
Перед переходом к **`108`** прочитай `plan_review_latest.json` с диска и проверь машинно:

- ключи **`plan_contract_version`** и **`missing_w14_plan_sections`** присутствуют;
- если `stage_status` == `approved`: **`plan_contract_version`** == **`"2026-05-w14-style"`** и **`missing_w14_plan_sections`** — пустой массив **`[]`**;
- если любое условие нарушено — **не** запускай `108`; оформи **blocker** (provenance `107` / несогласованный JSON) или один повторный вызов `107`, если это сбой записи файла, без правки плана из чата `100`.
...
- `context/artifacts/target_doc/plan_review_latest.json` (валидный JSON, `role` = `107_implementation_plan_reviewer`, `stage_status` = `approved`, путь плана согласован, **`plan_contract_version`** = **`2026-05-w14-style`**, **`missing_w14_plan_sections`** = **`[]`**);
...
0a. **Пакет `context/algorithms/<pkg>/`:** если утверждаемый канон — каталог-пакет с `INDEX.md` (не одиночный файл `context/algorithms/<name>.md`), путь **`context/algorithms/<pkg>/donors/INDEX.md`** обязан существовать до `completion_allowed=true` (owner содержимого — **`104`** при split/публикации; см. `.cursor/agents/104_target_doc_author.md`). Если файла нет — **не** закрывай completion; верни **`104`** с handoff «создать `donors/INDEX.md` по synthesis и `context/artifacts/target_doc/donor/*`». Если в `research_waves` не было **ни одного** job `donor_repo`, в `donors/INDEX.md` достаточно секции **Not researched** с одной строкой причины. Для одиночного файла канона без каталога-пакета пункт **0a** не применяется.
...
- `context/artifacts/target_doc/plan_review_latest.json` существует, валидный JSON, `"role":"107_implementation_plan_reviewer"`, `"stage_status":"approved"`, `implementation_plan_file` согласован с планом, **`plan_contract_version":"2026-05-w14-style"`**, **`missing_w14_plan_sections":[]`**;
- если канон опубликован как пакет `context/algorithms/<pkg>/` с `INDEX.md`, существует **`context/algorithms/<pkg>/donors/INDEX.md`** (см. Approval gate **0a**);
...
- если JSON от **`107`**: помимо `role`/`stage_status`/`implementation_plan_file`, проверь наличие **`plan_contract_version`** и **`missing_w14_plan_sections`**; при `stage_status=approved` значения строго **`2026-05-w14-style`** и пустой массив **`[]`** для `missing_w14_plan_sections`.
