...
| [`broker-memory-work-inject.md`](broker-memory-work-inject.md) | UC 2.4: trace-тройка Work → Memory → `context.memory_injected` v2; pathless v1; post-pipeline Memory. **W14:** SoT `agent_memory_result`, **`memory_continuation_required`**, `propose_links` + PAG write, внешние события / журнал durability, OR-013; **UC-05** `runtime.cancel_active_turn` через broker. |
| [`runtime-event-contract.md`](runtime-event-contract.md) | **D-OBS-1:** whitelist compact-событий AW↔AM + `memory.command.normalized`, `session.cancelled` / `action.cancelled` (UC-05); единый литерал `reason`=`continuation`; ссылка на каталог `agent_memory.external_event.v1` / golden map (`ailit/agent_memory/contracts/agent_memory_external_events.py`); проверка `rg` в файле. |
| [`memory-query-context-init.md`](memory-query-context-init.md) | **UC-03:** CLI `ailit memory init` → `memory.query_context`: payload `memory_init`, `goal`, `project_root`, `workspace_projects`, запрет непустых `path`/`hint_path`; коды выхода OR-011 (`ailit/agent_memory/cli/memory_init_cli_outcome.py`); VERIFY journal (`memory.result.returned` + `payload.status=complete`); разделение логов `cli_init` (`ailit-cli-*`) vs desktop flat. |
| [`pag-stable-indexing.md`](pag-stable-indexing.md) | PAG: запись не зависит от `goal`; merge C/B/A при инкрементальном sync; нейтральные W14 summaries; отсечка `summarize_b` по `child_basis_fingerprint`. |
