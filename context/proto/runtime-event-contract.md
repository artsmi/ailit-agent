...
- Каталог **`agent_memory.external_event.v1`** (не входит в таблицу D-OBS-1 выше): `ailit/agent_memory/contracts/agent_memory_external_events.py` — дискриминанты `event_type`, durable/ephemeral, **golden map** stdout/wire → поле `event=` в `compact.log` (регрессия `ailit/agent_memory/tests/test_g14_agent_memory_external_event_mapping.py`).
