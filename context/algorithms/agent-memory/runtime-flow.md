...
7. **`summarize_phase`** — внутренние LLM вызовы `summarize_c` / `summarize_b` как подкоманды (не отдельный planner round), с лимитами и явной политикой ошибок (ошибка summary **не** переводит весь запрос в `blocked`, если остаётся полезный результат → `partial`).
