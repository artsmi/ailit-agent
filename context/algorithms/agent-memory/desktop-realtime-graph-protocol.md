...
| **OR-010** | Несколько подключённых проектов к одному чату (1–5): 3D memory и протокол должны это поддерживать; **unified** сцена по умолчанию без cross-edges; подсветка по всем workspace namespace в store и per-node в unified; primary по конфигу — для broker handshake, не для отбрасывания highlight второго проекта. |
...
| **OR-012** | Изолированные проекты без общих рёбер отображаются корректно; правила компоновки (unified / separate / F) — в каноне `desktop-pag-graph-snapshot.md`. В 3D узлы из области входа (`merged`/slice/union) **отображаются полностью** после UC-04A; синтетические узлы/рёбра в проекции **запрещены**. |
...
| **OR-014** | Появление ноды в PAG: **первая реальная связь к родителю** + **факт существования** узла; суммаризация **не обязательна**. В 3D узел виден, если он попал во **вход проекции** (slice/union); рёбра без обоих концов в множестве id отбрасываются (UC-04A). Placeholder-узлы подсветки **могут** быть на сцене без рёбер; акцент мысли агента — **подсветка**. |
...
- Статус «идёт recall AgentMemory» в чате — **эвристика по trace** (`memory.query_context` и последующие topic-события), не отдельный канал. **Закрытие recall (C1):** ответ Memory→Work считается завершением ожидания, если это `service.request` с `ok: boolean` и полезная нагрузка распознаётся как ответ на `memory.query_context`: явный `payload.service === "memory.query_context"` **или** (прод-форма) непустой объект `memory_slice` и/или `agent_memory_result` без дублирования `service` в JSON.
...
- **«Думает» / ход агента:** флаг `agentTurnInProgress` в проекции trace (`desktop/src/renderer/runtime/chatTraceProjector.ts`) сбрасывается на финальных событиях хода (`assistant_final`, `turn_completed`, `turn_failed`, отмена сессии и т.д.), а **не** по «концу видимого текста» в панели чата. Промежуточные tool/model-шаги могут держать флаг `true`; отдельно UI гасит фразу «думает» на окне recall памяти (`brokerMemoryRecallActive`), чтобы не дублировать статус с recall.
...
- **Проекция 3D:** нормализация концов рёбер (защита от мутаций force-graph) → UC-04A (отбросить рёбра без обоих концов в множестве id узлов). **Полный набор узлов** входа остаётся на сцене; reachability-gate до A **не** применяется. В `ForceGraph3D` уходит **клон** графа (`cloneMemoryGraphForForceGraphRender`), чтобы не портить `merged` в store. Синтетические узлы/рёбра не добавляются.
...
- **`ensureHighlightNodes`** добавляет **placeholder-узлы** без рёбер для id из highlight в `merged` (используется 2D как список). В 3D те же узлы **могут** отображаться, если присутствуют во входе проекции; подсветка задаёт визуальный фокус.
- **Мульти-namespace подсветка (supersedes старого ограничения):** store заполняет `searchHighlightsByNamespace` для **каждого** workspace namespace (`applyHighlightFromTraceRows`); в режиме **unified** 3D использует подсветку по **`node.namespace`** (`usePerNodeNamespaceHighlight`). Политика конфига **`highlight_namespace_policy`** по-прежнему задаёт primary-цепочку для **broker handshake**, а не отбрасывает второй namespace из PAG highlight в UI.
...
| `pag.edge.upsert` | `namespace`; `rev`; `edges`: non-empty array; концы ребра — канон `from_node_id`/`to_node_id` и `edge_id`; Desktop `linkFromPag` также принимает `source_node_id`/`target_node_id`, короткие `from`/`to`, `id` вместо `edge_id` (G4). | Пустой `edges` |
...
9. **3D render:** `project(merged)` = нормализация концов рёбер → UC-04A; передать в рендерер **клон** результата. Все узлы входа (после slice/union) в `graphData`; подсветка — **лёгкий refresh** без смены ключа `ForceGraph3D` там, где позволяет OR-011 (см. remount при смене `graph_rev`).
...
3. **OR-012 Изоляция:** проекты без общих рёбер отображаются **корректно** (режим unified по умолчанию); синтетические узлы/рёбра в проекции **запрещены**. В 3D виден полный набор узлов области после UC-04A.
...
5. **OR-014 Появление ноды в 3D:** узел виден, если он есть во входе проекции (данные PAG / placeholder подсветки); **наличие суммаризации не обязательно**. Рёбра без обоих концов в id узлов не рисуются (UC-04A).
