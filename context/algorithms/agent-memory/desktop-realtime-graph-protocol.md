...
| **OR-012** | Изолированные проекты без общих рёбер отображаются корректно; правила компоновки и синтетический корень связности в проекции 3D — в каноне. |
...
| **OR-014** | Появление ноды: **первая реальная связь к родителю** + **факт существования** узла; суммаризация **не обязательна**; связь с PAG-дельтами и placeholder-узлами подсветки. |
...
- **Проекция:** фильтр «висячих» рёбер + при нескольких компонентах связности — **синтетический корень** `ailit:trace-conn-root:{namespace}` и рёбра к представителю компоненты (важно для изолированных подграфов OR-012).
...
- **`ensureHighlightNodes`** добавляет **placeholder-узлы** без рёбер для id из highlight, если узла ещё нет в merged — стык с OR-014 (визуал до полной PAG-связности).
...
- Запись на диск из main: `trace/*.jsonl`, `session/desk-diagnostic-*.log`, save dialog; PAG путь через CLI/env **без** отдельного desktop-конфига.
...
9. **3D render:** спроецировать merged (фильтр рёбер, синтетический корень для изолированных компонент); обновлять подсветку через **лёгкий refresh** без обязательной смены идентичности всего графа (целевое); при смене стратегии ключа виджета — минимизировать частоту remount (**target**, см. OR-011/013).
...
3. **OR-012 Изоляция:** проекты без общих рёбер отображаются **корректно**; допускается **синтетический корень связности** только для стабилизации force-layout в пределах **одной проекции namespace**, без ложных «продуктовых» рёбер PAG.
...
5. **OR-014 Появление ноды:** узел считается **достаточным для отображения** при **первой реальной связи к родителю** (ребро в PAG) и **факте существования** узла; **наличие суммаризации не обязательно**. Placeholder-узлы только для подсветки — **допустимы** как временный UX, но не заменяют правило связности PAG для «реального» узла.
...
# session_diag_dir:
#   {runtime_dir}/session/desk-diagnostic-<safe_chat>.log
# memory_journal_file:
