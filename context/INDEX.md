...
| [`algorithms/INDEX.md`](algorithms/INDEX.md) | Утверждённые target algorithms: человекочитаемые целевые алгоритмы, которые служат опорой для `start-feature` / `start-fix`. Внутри каталога — пакеты в т.ч. [`agent-memory/`](algorithms/agent-memory/INDEX.md), [`agents/`](algorithms/agents/INDEX.md), [`desktop/`](algorithms/desktop/INDEX.md) (граф памяти в Electron, 3dmvc), [`desktop-stack/`](algorithms/desktop-stack/INDEX.md) (чат, freeze, PAG rev). |
