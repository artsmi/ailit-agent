...
### D-TRACE-CONN-1: синтетический корень связности

- **Назначение:** связность отображаемого графа при trace-only / неполном PAG после применения фильтра рёбер ветки A.
- **Id синтетического узла:** один узел на отображаемую комбинацию `(session_id, namespace)` со стабильным id  
  `ailit:trace-conn-root:{namespace}`  
  (namespace в slug-ключе — как для предупреждений rev; префикс `ailit:` зарезервирован под синтетику и не должен пересекаться с реальными id PAG).
- **Рёбра:** от корня к одному выбранному представителю каждой orphan-компоненты (детерминированный выбор представителя — по канону в `architecture.md` §4).
- **Условие появления корня:** по умолчанию корень **вводится только если** после фильтра рёбер (ветка A) остаётся **более одной** связной компоненты; при одной связной компоненте и отсутствии изолированных нод без рёбер корень **не** добавляется (минимизация визуального шума — см. `architecture.md` §4 D-TRACE-CONN-1).
...
`MemoryGraph3DPage.tsx` — проекция `loadState`, `pagDatabasePresent` и `merged` в режимы: loading, error, empty при отсутствии БД, trace-only, empty, ready. Проекция рёбер в ForceGraph следует **UC-04 ветка A** и при необходимости **D-TRACE-CONN-1** (см. раздел «Канон» выше); реализация — якоря в `architecture.md` §13 (проекция рёбер UC-04A + `ensureTraceConnectivity`).
