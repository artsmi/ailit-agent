...
**Главные файлы:** `desktop/src/renderer/runtime/pagGraphSessionStore.ts`, `desktop/src/renderer/runtime/pagGraphObservabilityCompact.ts` (строки trace `pag_graph_rev_reconciled` / `pag_snapshot_refreshed`), `desktop/src/renderer/runtime/DesktopSessionContext.tsx`, `desktop/src/renderer/views/MemoryGraph3DPage.tsx`, `desktop/src/renderer/views/MemoryGraphPage.tsx` (2D glow из снимка).

## Канон, якоря и перекрёстные ссылки

Поведение снимка `PagGraphSessionSnapshot`, 3D-проекция, **UC-04A**, порядок **X→Y** и ожидаемая **идемпотентность** reconcile (без лишних дублей при повторном slice с тем же rev) — **в этом файле**: якоря [`#snapshot-uc-04a`](#snapshot-uc-04a), [`#snapshot-x-to-y`](#snapshot-x-to-y), [`#snapshot-idempotency-reconcile`](#snapshot-idempotency-reconcile). Schema-like контракты compact-событий (required / nullable / forbidden, ownership) — в [`context/proto/desktop-memory-3d-observability.md`](../proto/desktop-memory-3d-observability.md) (волна **1_2**); здесь — поведение снимка и имена для согласования с **1_2**.

[`context/artifacts/architecture.md`](../artifacts/architecture.md) — временный pipeline-артефакт под **активный** срез планирования (например G3 highlight); **не** используйте номера его § как канонические якоря для снимка / UC-04 / X→Y.

<a id="snapshot-uc-04a"></a>
...
- **Запрещено** одновременно вводить плейсхолдер-ноды для «висячих» концов (ветка **B**) в том же payload, что и фильтр A: в каноне зафиксирована **только ветка A** (см. [якорь UC-04A](#snapshot-uc-04a) в этом файле; трассировка UC-04 в постановке — `plan/18-desktop-memory-graph-3dmvc.md` при необходимости).
...
<a id="snapshot-x-to-y"></a>

### Порядок **X→Y** (graph rev mismatch)
...
Негативные сценарии Н1/Н2 и лимиты предупреждений — в этом подразделе [X→Y](#snapshot-x-to-y) и в тестах store (`pagGraphSessionStore.test.ts`).
...
- <span id="snapshot-idempotency-reconcile"></span>**Частота и идемпотентность:** не плодить reconcile при повторном slice с тем же rev; детали дебаунса — в [`context/proto/desktop-memory-3d-observability.md`](../proto/desktop-memory-3d-observability.md).
...
- Слияние в merged: `PagGraphSessionTraceMerge.applyHighlightFromTraceRows`; при инкременте без новых trace-строк highlight не пересчитывается (анти-мигание — см. D-HI-1 в `pagGraphSessionStore.ts`). **Дельты** merged / rev на **live** trace в UI идут через async `applyIncrementalBounded` (G19.3 / S-D3, см. [якорь ниже](#live-trace-incremental-deltas)). Для 2D `merged` может оставаться **суперсетом** с highlight-only узлами; **N_scene** в 3D строится отдельно через `MemoryGraphForceGraphProjector.project`.
...
- **Точка входа:** `desktop/src/renderer/runtime/DesktopSessionContext.tsx` — `useEffect` на `rawTraceRows` при `loadState === "ready"` и непустом хвосте после `lastAppliedTraceIndex`.
- **Merge:** async IIFE, `await PagGraphSessionTraceMerge.applyIncrementalBounded` в `pagGraphSessionStore.ts` — общий bounded контур с `afterFullLoad` (`applyPagGraphTraceDeltasBoundedInRange`, `yieldReplayChunkBound`).
- **Отмена устаревшего результата:** `pagGraphIncrementalMergeGenerationRef` (новый проход эффекта увеличивает счётчик; перед `setPagGraphBySession` сравнивается поколение); cleanup эффекта выставляет `cancelled` для прерывания async-пути.
- **Паритет:** синхронный `PagGraphSessionTraceMerge.applyIncremental` остаётся в API store и в Vitest как эталон одного прохода против bounded replay на том же хвосте trace (`pagGraphSessionStore.test.ts`, **S-D3**).
...
`MemoryGraph3DPage.tsx` — проекция `loadState`, `pagDatabasePresent` и `merged` в режимы: loading, error, empty при отсутствии БД, trace-only, empty, ready. Данные в ForceGraph: **UC-04 ветка A** на нормализованных концах рёбер + **клон** для рендерера (см. раздел «Канон, якоря и перекрёстные ссылки» и [UC-04A](#snapshot-uc-04a) выше); реализация — `desktop/src/renderer/runtime/memoryGraphForceGraphProjection.ts` (`MemoryGraphForceGraphProjector.project`).
...
- **Финальный gate §5.0 (итерация Memory 3D / PAG):** явный subset — **12** файлов Vitest, **80** тестов — команда и таблица в [`../tests/INDEX.md`](../tests/INDEX.md). Полный прогон `cd desktop && npm run test` (например финальный **11** после G19.3) подхватывает весь пакет `desktop/` — по `context/artifacts/test_report.md` пример: **28** файлов, **182** теста; числа **не** подменяют таблицу §5.0 без осознанного расширения gate.
- `pagGraphSessionStore.test.ts` — `pagDatabasePresent`, full load, `applyIncremental` / **`applyIncrementalBounded`**, дедуп `warnings` по rev, сценарий «Refresh» с согласованным срезом (регресс по 1.1/1.3); паритет incremental vs bounded replay (**S-D3**, в т.ч. `TC-G19.3-ApplyIncrementalSnapshotMatchesBoundedReplay`); покрывает контракты `pagGraphRevWarningFormat` через store.
